...
def main():
    ...
    parser.add_argument(
        "-l",
        "--list",
        help="Enables output to a seperate list file instead of overwriting server file. Will be placed in \out\map_list.txt. Do not use with --ServerFile",
        action="store_true",
    )
    parser.add_argument(
        "-p",
    ...
    print(
        ...
        "server file" if args.list is None else "list file",
    )

    if args.ServerFile is not None and not server_file_exists:
        print(
            f"Your server file {args.ServerFile} does not exist. Check the file path."
        )
        input()
        exit()
        ...
        print(
            "Creating directory and map list file if it does not exist."
            if args.list
            else "You have not specified a server file or set list file argument, defaulting to list file."
        )
    ...
    if final_map_file is not None:
        ...
        print(
            "New ",
            "server file " if not args.list and server_file_exists else "map list ",
            "generated! Find you new file here: ",
            args.ServerFile
            if not args.list and server_file_exists
            else f"{DIR}{MAP_LIST_DIR_PATH}{MAP_LIST_PATH}",
        )
    else:
        raise Exception(
            "final_map_file not opened. Contact dev for assistance here https://github.com/ScidaJ/MW2MapScramblerPython"
        )


def string_builder(
    args, random_map_list: dict, map_list: dict, map_file: TextIOWrapper
) -> str:
...
def validate_args(args):
    ...
    if args.list and args.ServerFile is not None:
        print(
            "--list set to true when ServerFile is provided. Do not use these together."
        )
        valid = False
